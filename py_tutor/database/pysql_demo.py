...
database = pymysql.connect(host="127.0.0.1",user= "root", password="123456", database=constants.db_name, charset='utf8')
def gen_xlsx():
    cursor = database.cursor()

    sql = "SELECT company ,COUNT(company),SUM(weight),SUM(weight*price) FROM grain_data  GROUP BY company"
    cursor.execute(sql)
    ...
    tem_excel = xlrd.open_workbook('D:/tpl.xls', formatting_info=True)
    ...
    new_excel.save('../dist/7月下旬统计表.xls')
